...
#### About the Website
This website is for anyone who may need a cocktail recipe for an event they are hosting or if they just want to experiment with any alcohol they have at home. 

The Top Shelf website allows the user to easily navigate by selecting an alcohol that they wish to be the base of their recipe. They are then presented with a list of cocktails that are can be produced from the selected Alcohol. Once the user selects a cocktail they are presented with a list of the ingredients required to make the cocktail, followed by the instructions on how to make it.

If a user comes across a new cocktail recipe that they wish to add to the website, they can easily do so. They need to navigate to the login page where they should register as a new user. Once registered, they need to login with their username and password. After they have logged in they should navigate to the 'Add a Cocktail' page where they can provide all the required information to add their favourite new cocktail to the website.

A logged in user who is returning to the website can view a list of all the cocktails that they have uploaded to the website in the past. From there they can edit an existing recipe if they have found a nicer recipe for the cocktail. They can also delete the recipe if they don't wish it to be on display.

A user who is looking for a non-alcoholic cocktail would navigate to the Mocktails section of the website from the home page, here they will find a list of the non-alcoholic cocktails.


## Features
 
### Existing Features
- View Cocktails by Category - From the homepage with one click a user can get a list of cocktails from their chosen category
- Register/Login - Users can easily register as a new user or login as an existing user from the login page.
- Logged in Users can fill in the form on the 'Add a cocktail' page to add a new cocktail tot eh website.
...
- [Jinga2](http://jinja.pocoo.org/)
    - The project uses the **Jinga2** templating engine to add dynamic content to the various webpages throughout the project. Jinga2 allows the use of loops and conditional statements. 

- [Materialize CSS](https://materializecss.com)
    - The project uses **Materialize CSS** for general page styling and layout.

- [jQuery](https://jQuery.com)
    - The project uses **jQuery** to make sure some of the Materialize CSS components (dropdown menus in forms, image sliders & modals) work correctly. jQuery was also used to allow extra ingredients and instructions be added to the 'Add a cocktail' form.
...
- [MongoDB](https://mongodb.com)
    - The project uses **MongoDB** to store all the recipes and user information. CRUD (create, read, update & delete) tasks are performed on the database when user’s login, register, add a new cocktail, delete a new cocktail etc.

- Bycrypt
    -This project uses Bycrypt to store and encrypted hashed version of a user’s password in the database. By hashing a user password, the password is stored more securely.
...
On the Add Cocktail page the user must fill in each element of the form before they can submit the new recipe to the database. I tried on numerous occasions to submit the form with a separate input missing from the form on each occasion. The form did not submit and a message is displayed to the user that they need to fill in all elements of the form.

The user can add extra ingredients or step inputs by using the buttons provided. I tested the adding and remove buttons and they worked as I wished.
...
I regularly committed changes to Github through the source control feature on Visual Studio Code. I used a short commit message to reflect each new change that was made in each commit. At last count I had over 60 commits on this project.
...
I had to create a Procfile and a requirements.txt file in the root of my project to deploy it successfully to Heroku. The Procfile tells Heroku what commands it needs to run. The reuqimrenets.txt file tells Heroku what Python libraries it needs to install for the project to run correctly.

Heroku also required some Environmental Variables to be added for the IP, PORT and the MONGO_URI which is the link to the MongoDB database.

I created 4 test users to help with testing the website functionality and populating the database with the usernames - david, susan, claire and tom. The password for each account is password. Each user has a number of cocktails added by them. The username david has the most cocktails added to the database.
